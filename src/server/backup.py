...
def backup(server, json_obj):
    request = BackupRequestPackage(server.backup_dir, json_obj)
...
def restore(server, json_obj):
    finished = json_obj.get('finished')
    if finished:
        server.window.log('Restore successfully')
        return
    index = int(json_obj.get('index'))
    res = RestoreResponsePackage(server.meta[index], len(server.meta))
    server.window.log(f'<span style="color: rgb(170, 241, 175);">{res.type}</span>: {res.real_path}')
    return res.toJson()

def old_restore(server, json_obj):
    return {"success": False, "message": "Not Implemented"}

def metadata(server):
    meta = []
    path = {
        'Novel': os.path.join(server.backup_dir, 'Data', 'Novel.json'),
        'Category': os.path.join(server.backup_dir, 'Data', 'Category.json'),
        'NovelCategory': os.path.join(server.backup_dir, 'Data', 'NovelCategory.json'),
        'Setting': os.path.join(server.backup_dir, 'Data', 'Setting.json'),
        'Chapter': os.path.join(server.backup_dir, 'Data', 'Chapters'),
        'Download': os.path.join(server.backup_dir, 'Data', 'Downloads'),
        'Plugin': os.path.join(server.backup_dir, 'Data', 'Plugins'),
        'Image': os.path.join(server.backup_dir, 'Images')
    }
    ...
    for type in ['Category', 'Novel', 'NovelCategory']:
        meta.append({
            'type': type,
            'real_path': path[type]
        })
    ...
    meta_file = os.path.join(server.backup_dir, 'metadata.json')

    with open(meta_file, 'w', encoding='utf-8') as f:
        f.write(json.dumps(meta, ensure_ascii=False))
    server.window.log('Backup successfully!')
